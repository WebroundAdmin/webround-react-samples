...
const ShopSidebar6 = ({ wr }: { wr: Wr }) => {
    ...
    const isDirty = JSON.stringify(selectedTagValueIds) !== JSON.stringify(appliedTagValueIds) ||
        JSON.stringify(selectedRangeFilters) !== JSON.stringify(appliedRangeFilters);
                        ...
                        <section key={tag.id}>
                            ...
                            {openSections[tag.id] && (
                                <div className="grid grid-cols-1 gap-px bg-[#d1d1d1]">
                                    {tag.values.map(val => {
                                        const isSelected = selectedTagValueIds?.includes(val.id);
                                        return (
                                            <button
                                                key={val.id}
                                                onMouseDown={(e) => { e.preventDefault(); wr.shop.toggleTag(val.id); }}
