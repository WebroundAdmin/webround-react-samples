 ...
 */
const RangeFilter2 = ({ filter, wr }: { filter: WrFilter; wr: Wr }) => {
    const isPrice = filter?.type === "price";
    const isDate = filter?.type === "date";

    //v1
    const value = wr.shop.selectedRangeFilters?.[filter.id]?.value ?? ["", ""];
    const cadence = wr.shop.sortOption?.cadence;

    // Sync input changes with the global store via core shop utility
    const handleChange = (index, val) => {
        const updated = [...value];
        updated[index] = isDate ? val : (val === "" ? "" : Number(val));
        wr.shop.setRangeFilter(filter.id, updated as [any, any], isPrice, cadence);
    };
